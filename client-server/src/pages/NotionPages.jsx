...
export default function NotionPages() {
  ...
  return (
    <div className="flex flex-1 min-h-screen">
      {/* Pages Sidebar */}
      <div className="w-56 bg-gray-50 border-r border-gray-100 flex flex-col flex-shrink-0">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
          <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Pages</span>
          <button onClick={createPage} className="w-5 h-5 flex items-center justify-center rounded text-gray-400 hover:text-gray-700 hover:bg-gray-200 transition text-lg">+</button>
        </div>

        <div className="flex-1 overflow-y-auto py-2">
          {pages.length === 0 ? (
            <p className="text-xs text-gray-300 text-center py-6 px-4">Tiada page lagi</p>
          ) : (
            pages.map((page) => (
              <div
                key={page.id}
                ...
                className={`group flex items-center gap-2 px-3 py-2 mx-2 rounded-lg cursor-pointer transition ${
                  activePage === page.id ? "bg-white shadow-sm text-gray-900" : "text-gray-500 hover:bg-white hover:text-gray-700"
                }`}
              >
        ...
        <div className="p-3 border-t border-gray-100">
          <button onClick={createPage} className="w-full flex items-center gap-2 px-3 py-2 text-xs text-gray-400 hover:text-gray-700 hover:bg-white rounded-lg transition">
      ...
      <div className="flex-1 overflow-y-auto">
        {!currentPage ? (
          <div className="flex flex-col items-center justify-center h-full text-center p-8">
            ...
            <p className="text-gray-400 text-sm mb-4">Pilih page atau buat yang baru</p>
            <button onClick={createPage} className="bg-gray-900 text-white text-sm px-5 py-2 rounded-xl hover:bg-gray-700 transition">
              + New Page
            </button>
          </div>
        ) : (
          <div className="max-w-3xl mx-auto px-8 py-12">
            ...
            {editingTitle ? (
              <input
                ...
                className="w-full text-3xl md:text-4xl font-bold text-gray-900 outline-none bg-transparent mb-6 border-b border-gray-200 pb-2"
                placeholder="Untitled"
              />
            ) : (
              <h1
                onClick={() => setEditingTitle(true)}
                className="text-3xl md:text-4xl font-bold text-gray-900 mb-2 cursor-text hover:opacity-70 transition"
              >
                {currentPage.title || <span className="text-gray-300">Untitled</span>}
              </h1>
            )}

            <p className="text-xs text-gray-300 mb-8">Dikemaskini {formatDate(currentPage.updatedAt)}</p>

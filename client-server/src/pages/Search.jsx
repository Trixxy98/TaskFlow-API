...
export default function Search({ tasks, onToggle, onDelete }) {
  ...
  return (
    <div className="flex-1 p-4 md:p-8 max-w-2xl mx-auto w-full">
      <div className="mb-8">
        <h2 className="text-2xl md:text-3xl font-bold text-gray-900">Search</h2>
        <p className="text-gray-400 text-sm mt-1">Cari task anda</p>
      </div>
      ...
      <div className="relative mb-6">
        ...
        <input
          ...
          autoFocus
          className="w-full bg-white border border-gray-200 text-gray-800 rounded-2xl pl-10 pr-4 py-3 text-sm outline-none focus:border-gray-400 shadow-sm transition"
        />
      </div>
      ...
      {query === "" ? (
        <div className="text-center py-16">
          ...
          <p className="text-gray-400 text-sm">Taip sesuatu untuk mula cari</p>
        </div>
      ) : results.length === 0 ? (
        <div className="text-center py-16">
          <p className="text-4xl mb-3">🔍</p>
          <p className="text-gray-400 text-sm">Tiada task dijumpai untuk "{query}"</p>
        </div>
      ) : (
        <div className="space-y-2">
          <p className="text-xs text-gray-400 mb-3">{results.length} result dijumpai</p>
          {results.map((task) => (
            <div
              key={task.id}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className={`text-sm ${task.status === "completed" ? "line-through text-gray-300" : "text-gray-800"}`}>
                  {task.title}
                </p>

  ...
  return (
    <div className="flex-1 p-4 md:p-8 max-w-4xl mx-auto w-full">
      <div className="mb-6">
        ...
        <p className="text-gray-400 text-sm mt-1">Tasks mengikut tarikh</p>
      </div>

      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-5">
        {/* Month navigation */}
        <div className="flex items-center justify-between mb-5">
          <button onClick={prevMonth} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 text-gray-500 transition">←</button>
          <h3 className="font-semibold text-gray-900 capitalize">{monthName}</h3>
          <button onClick={nextMonth} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 text-gray-500 transition">→</button>
        </div>
            ...
            return (
              <button
                ...
                className={`relative aspect-square flex flex-col items-center justify-start pt-1.5 rounded-xl text-sm transition ${
                  ...
                  selectedDay === day ? "bg-indigo-50 text-indigo-700 ring-1 ring-indigo-200" :
              ...
              >
                ...
                {dayTasks.length > 0 && (
                  <div className="flex gap-0.5 mt-0.5">
                    ...
                    {dayTasks.length > 1 && <div className={`w-1 h-1 rounded-full ${isToday(day) ? "bg-white/60" : "bg-gray-200"}`} />}
                  </div>
                )}
              </button>
            );
        ...
        <div>
          ...
          {selectedTasks.length === 0 ? (
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 text-center">
              <p className="text-gray-400 text-sm">Tiada task pada hari ini</p>
            </div>
          ) : (
            <div className="space-y-2">
              {selectedTasks.map((task) => (
                <div key={task.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm px-4 py-3 flex items-center gap-3">
                  <div className={`w-2 h-2 rounded-full flex-shrink-0 ${task.status === "completed" ? "bg-emerald-400" : "bg-amber-400"}`} />
                  <span className={`text-sm flex-1 ${task.status === "completed" ? "line-through text-gray-300" : "text-gray-800"}`}>{task.title}</span>
      ...
      {!selectedDay && (
        <div>
          <h3 className="text-sm font-semibold text-gray-700 mb-3">Upcoming Tasks</h3>
          {tasks.filter((t) => t.due_date && t.status !== "completed").length === 0 ? (
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 text-center">
              <p className="text-gray-400 text-sm">Tiada upcoming tasks</p>
            </div>
          ) : (
            <div className="space-y-2">
              {tasks
                ...
                .map((task) => {
                  ...
                  return (
                    <div key={task.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm px-4 py-3 flex items-center gap-3">
                      ...
                      <span className={`text-xs font-medium ${diff < 0 ? "text-red-500" : diff === 0 ? "text-amber-500" : "text-gray-400"}`}>
